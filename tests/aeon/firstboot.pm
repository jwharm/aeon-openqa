...
sub run {
    ...
    assert_screen 'boot-enter-passphrase', 600;
    type_string $encryption_passphrase;
    send_key 'ret';

    # Aeon will boot and show a Welcome screen with the language selection
    assert_screen 'firstboot-language-selection', 600;

    # Select default language and keyboard layout
    assert_and_click 'firstboot-language-selection';
    assert_and_click 'firstboot-keyboard-layout';

    # Set timezone
    assert_and_click 'firstboot-timezone-1';
    ...
    assert_and_click('firstboot-timezone-2');
    ...
    # User setup
    assert_screen 'firstboot-aboutyou';
    type_string $testapi::username;
    send_key 'ret';
    assert_screen 'firstboot-password-1';
    type_string $testapi::password;
    send_key 'tab';
    send_key 'tab';
    type_string $testapi::password;
    assert_and_click 'firstboot-password-2';

    # Complete
    assert_and_click 'firstboot-complete';

    # Wait until the Aeon Welcome message appears
    assert_screen 'firstboot-applications-1', 600;
    assert_and_click 'firstboot-applications-1';

    # Click Customize
    assert_and_click 'firstboot-applications-2';

    # Click OK to start installing the default applications
    assert_and_click 'firstboot-applications-3';

    # Installing the default applications takes a while.
    # Press 'ctrl' every 60 seconds for 30 minutes to avoid the screen lock.
    send_key_until_needlematch 'firstboot-done', 'ctrl', 30, 60;
